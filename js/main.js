...
(function () {
  ...
  async function loadRemoteData() {
    try {
      ...
      // Public site only reads data; no sign-in is required.
      const menuSnap = await fsMod.getDocs(fsMod.collection(db, "menu"));
      if (!menuSnap.empty) {
        const groups = new Map();
        menuSnap.forEach((d) => {
          const x = d.data();
          const id = x.categoryId || String(x.category || "Other").toLowerCase().replace(/[^a-z0-9]+/g, "-");
          if (!groups.has(id)) groups.set(id, { id, name: x.category || "Other", icon: x.icon || "extra", items: [] });
          groups.get(id).items.push({ name:x.name||"", price:x.price||"", description:x.description||"", image:x.image||"" });
        });
      ...
      if (!offersSnap.empty) {
        REMOTE_OFFERS = [];
        offersSnap.forEach((d) => REMOTE_OFFERS.push(d.data()));
      }
    } catch (e) {
      // Keep the original bundled data if Firestore is not readable.
