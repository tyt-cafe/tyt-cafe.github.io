...
(function () {
  ...
  /* Selects a category tab by its data-target id, updates the active
     state + a11y attributes, scrolls the tab into view, and re-runs the
     filter with the existing switch animation. Shared by tab clicks and
     the swipe gesture below so both behave identically. */
  ...
  /* ---------------------------------------------------------------------
     CATEGORY TABS: click/tap-to-select PLUS horizontal drag/swipe-to-
     scroll, without the two interfering with each other.

     - Mobile: the bar already has `overflow-x: auto`, so finger swiping
       scrolls it via native touch scrolling. Native scrolling already
       suppresses the click on a tab the browser considers "scrolled
       past", so tapping a tab still selects it normally.
     - Desktop: `overflow-x: auto` has no built-in mouse-drag support, so
       we add it by hand using Pointer Events (mouse only — touch/pen
       keep native scrolling untouched).

     To keep a genuine click working while a drag doesn't accidentally
     select a category, we only start actually scrolling once the mouse
     has moved past a small threshold — until then it's treated as a
     plain click. Once a drag is confirmed we capture the pointer (so
     the drag keeps tracking correctly even if the mouse leaves the tab
     bar) and swallow the click event that follows, so releasing the
     mouse after a drag never triggers `selectCategory`.
  --------------------------------------------------------------------- */
  ...
  function bindMenuTabsListener() {
    ...
    menuTabs.addEventListener("click", (e) => {
      if (menuTabsDragging) {
        // This click is the tail end of a drag gesture, not a real tap —
        // consume it so it can't select a category.
        e.preventDefault();
        e.stopPropagation();
  ...
  function bindMenuTabsDragScrollListener() {
    ...
    menuTabs.addEventListener("pointermove", (e) => {
      ...
      if (!menuTabsDragging) {
        if (Math.abs(dx) <= DRAG_MOVE_THRESHOLD) return; // still within click tolerance
        // Threshold crossed — this is now a real drag. Capture the
        // pointer so movement keeps being tracked even if the mouse
        // strays outside the tab bar while dragging.
    ...
    const endDrag = (e) => {
      ...
      if (menuTabsDragging) {
        ...
        // menuTabsDragging itself is left true here on purpose: the
        // browser fires "click" right after "pointerup", and the click
        // handler above needs to see it was a drag so it can swallow
        // that trailing click. It resets the flag once it does.
      }
  ...
  /* ---------------------------------------------------------------------
     SWIPE BETWEEN MENU CATEGORIES (touch AND mouse/desktop): swiping (or
     click-dragging with a mouse) over the menu items moves to the
     next/previous tab, same as tapping a tab. Direction is flipped for
     RTL (Arabic) so it feels natural either way.

     Built on Pointer Events so a single set of listeners handles touch,
     mouse, and pen alike. Only reacts on pointerup (no pointermove/
     preventDefault), so normal vertical page scrolling and text
     selection are never interrupted.
  --------------------------------------------------------------------- */
  ...
  function bindMenuSwipeListener() {
    ...
    menuContainer.addEventListener("pointerdown", (e) => {
      // Only the primary mouse button (or a touch/pen contact) starts a drag.
      if (e.pointerType === "mouse" && e.button !== 0) return;
      dragActive = true;
      activePointerId = e.pointerId;
    ...
    // If the pointer leaves the container/window mid-drag (e.g. mouse
    // released outside it) or the gesture is cancelled, just reset —
    // don't fire a false-positive swipe.
