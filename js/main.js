...
(function () {
  ...
  /* ---------------------------------------------------------------------
     FIREBASE DATA
     ---------------------------------------------------------------------
     Firestore is the source of truth once a collection contains data.
     The admin writes one document at a time, so editing/deleting one item
     never replaces the whole collection. If a collection is empty or
     unreadable, the original bundled JS data remains the fallback.
  --------------------------------------------------------------------- */
  ...
  async function loadRemoteData() {
    try {
      ...
      if (!menuSnap.empty) {
        ...
        REMOTE_MENU = Array.from(groups.values());
      }
      ...
      section.innerHTML = `
        ...
        <div class="menu-grid">
          ${cat.items
            .map(
              (item) => `
            <div class="menu-item" data-name="${escapeHtml(String(item.name || "").toLowerCase())}" data-desc="${escapeHtml(String(item.description || "").toLowerCase())}">
  ...
  function renderOffers() {
    ...
    const offersSource = REMOTE_OFFERS || ((typeof OFFERS_DATA !== "undefined") ? OFFERS_DATA : []);
    ...
    offersGrid.innerHTML = offersSource.map((offer) => {
      ...
      const media = offer.image
        ...
        : `<span class="offer-icon">${escapeHtml(offer.icon || "☕")}</span>`;
      const discount = offer.discount ? `<span class="offer-discount">${escapeHtml(offer.discount)}</span>` : "";
      const oldPrice = offer.oldPrice ? `<span class="offer-price-old">${escapeHtml(offer.oldPrice)} EGP</span>` : "";

      return `
        <article class="offer-card" data-reveal>
          <div class="offer-media">
            ${media}
            ${offer.badge ? `<span class="${badgeClass}">${escapeHtml(offer.badge)}</span>` : ""}
            ${discount}
          </div>
          <div class="offer-body">
            ...
            <div class="offer-price-row">
              <span class="offer-price-new">${escapeHtml(offer.newPrice)} EGP</span>
              ${oldPrice}
            </div>
            <a href="#location" class="btn btn-primary">${escapeHtml(offer.cta || "Order Now")}</a>
          </div>
        </article>`;
    }).join("");
