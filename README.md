...
├── storage.rules         → (legacy, unused now — image uploads moved to Supabase Storage, see SUPABASE_SETUP.md)
└── README.md
```
...
device, and press **حفظ (Save)**. The photo uploads to Supabase Storage
(see below), its URL is saved on that one item's document, and it
appears on that item's card in the live Menu automatically. Each item
keeps its own separate photo — uploading one never affects any other
item or category.

**One-time setup required:** image uploads use Supabase Storage, not
Firebase Storage — Google now requires the paid Blaze plan just to
enable Firebase Storage, even for free-tier usage, so this project uses
Supabase's free tier instead (no card needed). Everything else (menu
data, admin login) still runs on Firebase. Follow `SUPABASE_SETUP.md` in
this folder once to create a free Supabase project and paste its URL +
key into `admin.html`. Until that's done, uploads from the Admin Panel
will show an error explaining the same thing.
...
## Update: menu strip + new logo
- Menu section now uses the horizontal "strip" menu (tabs, arrows, click-to-reveal cards, search) and webp photos.
- New logo set: `logo.png`, `logo-icon.png` (navbar/footer), `logo-drinking.png` (hero sip animation), `favicon.png`.
- The rest of the one-page site (hero text, about, gallery, offers, comments, location, contact) is unchanged.
